...
namespace OpenWifi {
	static ORM::FieldVec BaseTokenDB_Fields{ORM::Field{"token", ORM::FT_TEXT, 0, true},
	...
	static ORM::IndexVec MakeIndices(const std::string &shortname) {
		return ORM::IndexVec{
			{std::string(shortname + "_user_id_index"),
			 ORM::IndexEntryVec{{std::string("userName"), ORM::Indextype::ASC}}},
			{std::string(shortname + "_refresh_index"),
			 ORM::IndexEntryVec{{std::string("refreshToken"), ORM::Indextype::ASC}}}};
	}

	BaseTokenDB::BaseTokenDB(const std::string &Name, const std::string &ShortName,
							 OpenWifi::DBType T, Poco::Data::SessionPool &P, Poco::Logger &L,
							 TokenCache *Cache, [[maybe_unused]] bool Users)
		: DB(T, Name.c_str(), BaseTokenDB_Fields, MakeIndices(ShortName), P, L, ShortName.c_str(),
			 Cache) {}

	bool BaseTokenDB::AddToken(std::string &UserID, std::string &Token, std::string &RefreshToken,
							   [[maybe_unused]] std::string &TokenType, uint64_t Expires,
							   uint64_t TimeOut) {
		SecurityObjects::Token T{.token = Token,
								 .refreshToken = RefreshToken,
								 .tokenType = "Bearer",
								 .userName = UserID,
								 .created = (uint64_t)OpenWifi::Now(),
								 .expires = Expires,
								 .idleTimeout = TimeOut,
								 .revocationDate = 0};
	...
	bool BaseTokenDB::Upgrade([[maybe_unused]] uint32_t from, uint32_t &to) {
		std::vector<std::string> Statements{"alter table " + TableName_ +
											" add column lastRefresh BIGINT default 0;"};
	...
	bool BaseTokenDB::GetToken(std::string &Token, SecurityObjects::WebToken &WT,
							   std::string &UserId, uint64_t &RevocationDate) {
	...
	bool BaseTokenDB::RefreshToken(const std::string &OldToken, const std::string &NewToken,
								   const std::string &NewRefreshToken, uint64_t LastRefresh) {
		SecurityObjects::Token T;
	...
	bool BaseTokenDB::RevokeToken(std::string &Token) { return DeleteRecord("token", Token); }
	...
	TokenCache::TokenCache(unsigned Size, unsigned TimeOut, [[maybe_unused]] bool Users)
		...
		CacheByToken_ = std::make_unique<Poco::ExpireLRUCache<std::string, SecurityObjects::Token>>(
			Size, TimeOut);
	}
	...
	bool TokenCache::GetFromCache(const std::string &FieldName, const std::string &Value,
								  SecurityObjects::Token &R) {
		std::lock_guard M(Mutex_);
	...
	void TokenCache::Delete(const std::string &FieldName, const std::string &Value) {
		...
		if (FieldName == "token") {
			...
			CacheByToken_->forEach(
				[&TokenToRemove, Value](const std::string &Key,
										const SecurityObjects::Token &TokenRecord) {
...
template <>
void ORM::DB<OpenWifi::TokenRecordTuple, OpenWifi::SecurityObjects::Token>::Convert(
	const OpenWifi::TokenRecordTuple &T, OpenWifi::SecurityObjects::Token &U) {
	U.token = T.get<0>();
...
template <>
void ORM::DB<OpenWifi::TokenRecordTuple, OpenWifi::SecurityObjects::Token>::Convert(
	const OpenWifi::SecurityObjects::Token &U, OpenWifi::TokenRecordTuple &T) {
	T.set<0>(U.token);
