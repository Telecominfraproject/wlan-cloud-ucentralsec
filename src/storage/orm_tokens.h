...
namespace OpenWifi {
	...
	*/
	...
	typedef Poco::Tuple<std::string, // token
	...
	class TokenCache : public ORM::DBCache<SecurityObjects::Token> {
	  public:
		...
		bool GetFromCache(const std::string &FieldName, const std::string &Value,
						  SecurityObjects::Token &R) override;
	...
	class BaseTokenDB : public ORM::DB<TokenRecordTuple, SecurityObjects::Token> {
	  public:
		BaseTokenDB(const std::string &name, const std::string &shortname, OpenWifi::DBType T,
					Poco::Data::SessionPool &P, Poco::Logger &L, TokenCache *Cache, bool User);
		virtual ~BaseTokenDB() {}

		bool AddToken(std::string &UserId, std::string &Token, std::string &RefreshToken,
					  std::string &TokenType, uint64_t Expires, uint64_t TimeOut);
		...
		bool GetToken(std::string &Token, SecurityObjects::WebToken &WT, std::string &UserId,
					  uint64_t &RevocationDate);
		bool RefreshToken(const std::string &OldToken, const std::string &NewToken,
						  const std::string &NewRefreshToken, uint64_t LstRefresh);
