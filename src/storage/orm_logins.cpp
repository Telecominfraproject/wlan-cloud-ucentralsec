...
namespace OpenWifi {
	...
	static ORM::FieldVec LoginDB_Fields{ORM::Field{"session", 64, true},
	...
	static ORM::IndexVec MakeIndices(const std::string &shortname) {
		return ORM::IndexVec{{std::string(shortname + "_userid_index"),
	...
	LoginDB::LoginDB(const std::string &TableName, const std::string &Shortname, OpenWifi::DBType T,
		...
		: DB(T, TableName.c_str(), LoginDB_Fields, MakeIndices(Shortname), P, L,
			 Shortname.c_str()) {}

	static std::string MakeSessionId(const std::string &token) { return Utils::ComputeHash(token); }

	void LoginDB::AddLogin(const std::string &userId, const std::string &email,
...
template <>
void ORM::DB<OpenWifi::LoginInfoRecordTuple, OpenWifi::SecurityObjects::LoginRecordInfo>::Convert(
	const OpenWifi::LoginInfoRecordTuple &R, OpenWifi::SecurityObjects::LoginRecordInfo &P) {
	P.sessionId = R.get<0>();
	P.userId = R.get<1>();
	P.email = R.get<2>();
	P.login = R.get<3>();
...
template <>
void ORM::DB<OpenWifi::LoginInfoRecordTuple, OpenWifi::SecurityObjects::LoginRecordInfo>::Convert(
	const OpenWifi::SecurityObjects::LoginRecordInfo &P, OpenWifi::LoginInfoRecordTuple &R) {
	R.set<0>(P.sessionId);
	R.set<1>(P.userId);
	R.set<2>(P.email);
	R.set<3>(P.login);
