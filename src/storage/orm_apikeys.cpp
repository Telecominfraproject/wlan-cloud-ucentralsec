...
namespace OpenWifi {
	static ORM::FieldVec ApiKeyDB_Fields{ORM::Field{"id", 36, true},
	...
	static ORM::IndexVec MakeIndices(const std::string &shortname) {
		return ORM::IndexVec{{std::string(shortname + "_username_index"),
							  ORM::IndexEntryVec{{std::string("userUuid"), ORM::Indextype::ASC}}},
							 {std::string(shortname + "_apikey_index"),
							  ORM::IndexEntryVec{{std::string("apiKey"), ORM::Indextype::ASC}}}};
	};

	ApiKeyDB::ApiKeyDB(const std::string &TableName, const std::string &Shortname,
					   OpenWifi::DBType T, Poco::Data::SessionPool &P, Poco::Logger &L)
		: DB(T, TableName.c_str(), ApiKeyDB_Fields, MakeIndices(Shortname), P, L,
			 Shortname.c_str()) {}

	bool ApiKeyDB::RemoveAllApiKeys(const std::string &user_uuid) {
		SecurityObjects::ApiKeyEntryList Keys;
		if (StorageService()->ApiKeyDB().GetRecords(0, 500, Keys.apiKeys,
...
template <>
void ORM::DB<OpenWifi::ApiKeyRecordTuple, OpenWifi::SecurityObjects::ApiKeyEntry>::Convert(
	const OpenWifi::ApiKeyRecordTuple &In, OpenWifi::SecurityObjects::ApiKeyEntry &Out) {
	Out.id = In.get<0>();
	...
	Out.rights.acls =
		OpenWifi::RESTAPI_utils::to_object_array<OpenWifi::SecurityObjects::ApiKeyAccessRight>(
			In.get<8>());
...
template <>
void ORM::DB<OpenWifi::ApiKeyRecordTuple, OpenWifi::SecurityObjects::ApiKeyEntry>::Convert(
	const OpenWifi::SecurityObjects::ApiKeyEntry &In, OpenWifi::ApiKeyRecordTuple &Out) {
	Out.set<0>(In.id);
