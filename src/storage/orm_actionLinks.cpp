...
namespace OpenWifi {
	static ORM::FieldVec ActionLinksDB_Fields{ORM::Field{"id", 36, true},
	...
	ActionLinkDB::ActionLinkDB(const std::string &Name, const std::string &ShortName,
	...
	bool ActionLinkDB::Upgrade([[maybe_unused]] uint32_t from, uint32_t &to) {
		std::vector<std::string> Statements{"alter table " + TableName_ +
											" add column userAction BOOLEAN default true;"};
	...
	bool ActionLinkDB::CreateAction(SecurityObjects::ActionLink &A) { return CreateRecord(A); }
	...
	bool ActionLinkDB::DeleteAction(std::string &ActionId) { return DeleteRecord("id", ActionId); }
...
template <>
void ORM::DB<OpenWifi::ActionLinkRecordTuple, OpenWifi::SecurityObjects::ActionLink>::Convert(
	const OpenWifi::ActionLinkRecordTuple &T, OpenWifi::SecurityObjects::ActionLink &U) {
	U.id = T.get<0>();
	U.action = T.get<1>();
	U.userId = T.get<2>();
...
template <>
void ORM::DB<OpenWifi::ActionLinkRecordTuple, OpenWifi::SecurityObjects::ActionLink>::Convert(
	const OpenWifi::SecurityObjects::ActionLink &U, OpenWifi::ActionLinkRecordTuple &T) {
	T.set<0>(U.id);
	T.set<1>(U.action);
	T.set<2>(U.userId);
