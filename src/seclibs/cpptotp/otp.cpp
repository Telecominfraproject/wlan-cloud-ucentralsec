...
namespace CppTotp {
	...
	// uint32_t hotp(const Bytes::ByteString & key, const Bytes::ByteString & msg, size_t
	// digitCount, HmacFunc hmacf)
	uint32_t hotp(const Bytes::ByteString &key, uint64_t counter, size_t digitCount,
				  HmacFunc hmacf) {
		...
		uint32_t ret =
			(fourWord[0] << 24) | (fourWord[1] << 16) | (fourWord[2] << 8) | (fourWord[3] << 0);
	...
	uint32_t totp(const Bytes::ByteString &key, uint64_t timeNow, uint64_t timeStart,
				  uint64_t timeStep, size_t digitCount, HmacFunc hmacf) {
...
int main(void) {
	...
	std::cout << (hotp(key, 0, digitsH) == 755224) << (hotp(key, 1, digitsH) == 287082)
			  << (hotp(key, 2, digitsH) == 359152) << (hotp(key, 3, digitsH) == 969429)
			  << (hotp(key, 4, digitsH) == 338314) << (hotp(key, 5, digitsH) == 254676)
			  << (hotp(key, 6, digitsH) == 287922) << (hotp(key, 7, digitsH) == 162583)
			  << (hotp(key, 8, digitsH) == 399871) << (hotp(key, 9, digitsH) == 520489)
			  ...
			  << (totp(key, 20000000000, start, step, digitsT) == 65353130) << std::endl;
