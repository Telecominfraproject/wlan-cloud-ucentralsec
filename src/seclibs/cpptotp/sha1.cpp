...
namespace CppTotp {
	...
	Bytes::ByteString sha1(const Bytes::ByteString &msg) {
		...
		for (size_t i = 0; i < bstr.size() / 64; ++i) {
			...
			for (j = 0; j < 16; ++j) {
				words[j] = (chunk[4 * j + 0] << 24) | (chunk[4 * j + 1] << 16) |
						   (chunk[4 * j + 2] << 8) | (chunk[4 * j + 3] << 0);
			}
	...
	Bytes::ByteString hmacSha1(const Bytes::ByteString &key, const Bytes::ByteString &msg,
							   size_t blockSize = 64);
...
int main(void) {
	...
	const uint8_t *strDog =
		reinterpret_cast<const uint8_t *>("The quick brown fox jumps over the lazy dog");
	const uint8_t *strCog =
		reinterpret_cast<const uint8_t *>("The quick brown fox jumps over the lazy cog");
	...
	std::cout << (Bytes::toHexString(shaEmpty) == "da39a3ee5e6b4b0d3255bfef95601890afd80709")
			  << std::endl
			  << (Bytes::toHexString(shaDog) == "2fd4e1c67a2d28fced849ee1bb76e7391b93eb12")
			  << std::endl
			  << (Bytes::toHexString(shaCog) == "de9f2c7fd25e1b3afad3e85a0bd17d9b100db4b3")
			  << std::endl
			  << std::endl
			  << (Bytes::toHexString(hmacShaEmpty) == "fbdb1d1b18aa6c08324b7d64b71fb76370690e1d")
			  << std::endl
			  << (Bytes::toHexString(hmacShaKeyDog) == "de7c9b85b8b78aa6bc8a7a36f70a90701c9db4d9")
			  << std::endl
			  << std::endl;
